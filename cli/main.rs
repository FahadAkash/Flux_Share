...
#[derive(Parser, Debug)]
#[command(name = "fluxshare")]
#[command(about = "P2P file transfer CLI for FluxShare")]
struct Cli {
    #[command(subcommand)]
    command: Command,
}
...
enum Command {
    Send {
        ...
        #[arg(
            long = "file",
            value_name = "PATH",
            help = "File to send (legacy --file)",
            hide = true
        )]
        file_flag: Option<PathBuf>,
        #[arg(
            value_name = "ROOM_ID_OR_URL",
            help = "Room ID or full room URL (optional, supports #k=...)"
        )]
        room_input: Option<String>,
        #[arg(
            long = "room-id",
            value_name = "ROOM_ID_OR_URL",
            help = "Room ID or full room URL (legacy --room-id)",
            hide = true
        )]
    ...
    Receive {
        #[arg(
            value_name = "ROOM_ID_OR_URL",
            help = "Room ID or full room URL (supports #k=...)"
        )]
        room_input: Option<String>,
        #[arg(
            long = "room-id",
        ...
        #[arg(
            long,
            value_name = "KEY",
            help = "Base64url decryption key (overrides #k=...)"
        )]
        key: Option<String>,
...
async fn main() -> Result<()> {
    ...
    match cli.command {
        ...
        Command::Receive {
            ...
            let room_input = room_id.or(room_input).ok_or_else(|| {
                anyhow!("Room ID or URL is required (usage: receive <ROOM_ID_OR_URL>)")
            })?;
            run_receive(
                &room_input,
                &output_dir,
...
async fn create_offerer_peer(
    ...
    dc.on_open(Box::new(move || {
        ...
        Box::pin(async move {
            ...
            let _ = send_tx.send(ClientMessage::TransferDone {
...
async fn handle_offer_candidate(
    peer: Arc<OffererPeer>,
    sid: u64,
    candidate: RTCIceCandidateInit,
) -> Result<()> {
    ...
    if guard.remote_desc_set {
        ...
        guard
            .pending_candidates
            .push(PendingCandidate { sid, candidate });
    }
    Ok(())
}
                        ...
                        match parsed {
                            DataMessage::Meta {
                                ...
                                if encrypted && guard.crypto.is_none() {
                                    log_line(
                                        "[recv] error",
                                        "encrypted files need a decryption key",
                                    );
                                    return;
                                }
                                ...
                                match File::create(&path).await {
                                    Ok(file) => {
                                        ...
                                        log_line(
                                            "[recv] meta",
                                            &format!("{safe_name} ({mime}, {size} bytes)"),
                                        );
                                    }
                                    Err(err) => {
